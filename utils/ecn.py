...
def main():
     ...
     #queryset = np.asarray([[float(v) for v in line.strip().split(',')] for line in open(args.query)])
     #testset = np.asarray([[float(v) for v in line.strip().split(',')] for line in open(args.test)])

     queryset = np.load('/home/lidg/Competition/BOT/query.npy')
     testset = np.load('/home/lidg/Competition/BOT/gallery.npy')
